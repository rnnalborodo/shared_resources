...
import es.upm.babel.ccjml.samples.boundedsemaphore.java.BoundedSemaphore;
import es.upm.babel.cclib.Tryer;

public abstract class TestBoundedSemaphore {
    
  // The share resource
  protected BoundedSemaphore resource = null;
  
  protected int BOUND = 3;
  ...
  // V can be performed after initialization
  @Test public void testVWorksAfterInit() {
    V v = new V();
    v.start();
  ...
  @Test public void testVUnblocksProperly() {
    ...
    assertTrue(trace + "-> " + p.call() + " shouldn't have blocked",
        !p.isBlocked());
    assertTrue(trace + "-> " + v[BOUND].call() + " should have unblocked",
        !v[BOUND].isBlocked());
  }
