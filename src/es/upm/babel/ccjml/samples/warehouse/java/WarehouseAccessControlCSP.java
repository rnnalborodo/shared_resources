...
public class WarehouseAccessControlCSP implements WarehouseAccessControl, CSProcess {
  ...
  Any2OneChannel chEnteringWarehouse[][]; //(N_WAREHOUSES*MAX_WEIGHT)
  Any2OneChannel chExitingWarehouse[]; //(N_WAREHOUSES)
  ...
  public void enterWarehouse(int n, int w) {
    ...
    chEnteringWarehouse[n][w-1].out().write(new WeightedRequest(n,w));
  }
  ...
  public void run() {
    /* 
     * Channel splitting positions (MAX_WEIGHT + N_WAREHOUSES*2)
     * inputs[0..N_WAREHOUSES] -> exitWarehouse
     * inputs[N_WAREHOUSES..] -> enterWarehouse
     */
    Guard[] inputs = new Guard[N_WAREHOUSES + (MAX_WEIGHT * N_WAREHOUSES)];
    for (int n = 0; n < N_WAREHOUSES; n++) {
      inputs[n] = chExitingWarehouse[n].in();
      // flattening enteringWarehouse channels
      for (int w = 0; w < MAX_WEIGHT; w++) {
        inputs[N_WAREHOUSES + n*MAX_WEIGHT+w] = chEnteringWarehouse[n][w].in();
      }
    }
    ...
    boolean[] syncCond = new boolean[N_WAREHOUSES + (MAX_WEIGHT * N_WAREHOUSES)];
    while (true) {
      // updating syncCond for enterWarehouse(0,w)
  	  for (int n = 0; n < N_WAREHOUSES; n++) {
  	    System.out.println(n);
  		  //exitWarehouse(n,w) CPRE
  	    syncCond[n] = ! (n != N_WAREHOUSES) || !occupied[n];
  	    // enterWarehouse(n,w) CPRE
  	    for (int w = 0; w < MAX_WEIGHT; w++) {
  	      syncCond[N_WAREHOUSES + n*MAX_WEIGHT+w] = weight[n] + (w+1) <= MAX_WEIGHT;
  	    }
  	  }
      ...
      if (chosenService < N_WAREHOUSES) { // exitWarehouse
        //@ assert chosenService - MAX_WEIGHT != N_WAREHOUSES ==> !occupied[chosenService];
