...
procedure Bb_Eo_Req_Entfam is
   ...
   private
      ...
      entry Delayed_Get
        (Request_Type)
        (Req : in Request_Type; What : out Item_Type);
   end Buffer;
   ...
   is
      entry Get
        (Req : in     Request_Type;
         What : out Item_Type)
      when True is              --  A weakened precondition can go here
      begin
         requeue Delayed_Get (Req);
      end Get;

      entry Delayed_Get
        (for I in Request_Type)
        (Req : in Request_Type; What : out Item_Type)
      when Item_Present and
      ((I = Even) = (Item mod 2 = 0)) is
      begin
         What := Item;
         Item_Present := False;
      end Delayed_Get;
